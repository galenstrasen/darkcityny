...
Usage in your posts:
[soundcloud]http://soundcloud.com/TRACK_PERMALINK[/soundcloud] 
Works also with set or group instead of track.
You can provide optional parameters height/width/params as follows
[soundcloud height="166" params="auto_play=true"]http....
...
class SoundcloudShortcode {
  ...
  private function getHTML($url, $iframe, $params, $width, $height) {
    ...
    if ($iframe) {
      $html = ' <!--[if IE]>
                <iframe width="' . $width . '" height="' . $height . '" scrolling="no" frameborder="no" src="' . $player_src . '"></iframe>
                <![endif]-->
                <!--[if !IE]>-->
                <iframe width="' . $width . '" height="' . $height . '" style="border-style:none; overflow:hidden; width:100%;" src="' . $player_src . '"></iframe>
                <!--![endif]-->';       

       } else {
      $html = '<object height="' . $height . '" width="' . $width . '"><param name="movie" value="' . $player_src . '" /><param name="allowscriptaccess" value="always" /><embed allowscriptaccess="always" height="' . $height . '" src="' . $player_src . '" type="application/x-shockwave-flash" width="' . $width . '" /></object>';
    }
    return $html;
  }
...
function soundcloud_shortcode_options() {
  if (!current_user_can('manage_options')) {
    wp_die( __('You do not have sufficient permissions to access this page.', 'gxg_textdomain') );
  }
...
<div class="wrap">
  ...
  <form method="post" action="options.php">
    ...
    <table class="form-table">
      ...
      <p class="submit">
        <input type="submit" class="button-primary" value="<?php _e('Save Changes', 'gxg_textdomain') ?>" />
      </p>
